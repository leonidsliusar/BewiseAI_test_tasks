...
async def exist_check(question_collection: list[dict], session: AsyncSession, unique_question: Optional[list[dict] | None] = None) -> set[int]:
...
async def create_question(questions_collection: list[dict]) -> list[dict]:  # make DB insertion and return previous saved entries
    ...
    async with async_session() as session:
        async with session.begin():  # run transaction
            ...
            repeatable_set = await exist_check(questions_collection, session)  # find repeat questions in db
            ...
            while repeatable_set:  # while id's not unique in db or not unique db and responses
                ...
                repeatable_set = await exist_check(repeatable_response, session, unique_question)  # keep check response for unique constraint in db in repeat requests
